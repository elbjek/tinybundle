        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {apps.map((app) => (
            <Link
              key={app.slug}
              href={`/${app.slug}`}
              className={`group flex flex-col items-center rounded-2xl px-6 py-9 text-center shadow-sm transition-transform duration-300 hover:-translate-y-1 hover:shadow-md ${app.cardClassName}`}
            >
