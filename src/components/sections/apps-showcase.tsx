      ...
      <div className="mx-auto w-full max-w-6xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {apps.map((app) => {
            ...
            const className = `flex flex-col items-center rounded-2xl px-6 py-9 text-center shadow-sm ${app.cardClassName} ${
              app.comingSoon && app.slug !== "tinyblooms"
                ? ""
                : "group transition-transform duration-300 hover:-translate-y-1 hover:shadow-md"
            }`;

            if (app.comingSoon && app.slug !== "tinyblooms") {
              return (
                <div key={app.slug} className={className} aria-label={`${app.name}, coming soon`}>
                  {body}
                </div>
              );
            }
